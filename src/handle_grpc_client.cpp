...
namespace remote {
    ClientGrpcHandle::ClientGrpcHandle(const std::string &connect_to_addr, const std::string &connect_to_port)
        ...
        EasyLoggerWithTrace(multi_master::path_log_client, EasyLogger::info).force_flush() << "connect to: " << connect_to_addr << ":" << connect_to_port;
    }
    ...
    std::string ClientGrpcHandle::Send(const std::string &json) {
        ...
        EasyLoggerWithTrace(multi_master::path_log_client, EasyLogger::info).force_flush() << "client send content:" << json;
#endif
        ...
        if (status.ok()) {
            ...
            EasyLoggerWithTrace(multi_master::path_log_client, EasyLogger::info).force_flush() << "grpc call error_details:" << status.error_details() << ", json:" << json.substr(0, 30);
#endif
